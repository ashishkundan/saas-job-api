...
Reservation/redelivery is lazy check-on-read (evaluated inside claim()) rather
than a background sweep -- there is no scheduler lifecycle to manage and the
in-memory dataset is small enough that an O(n) scan per poll is free. A fresh
receipt_token is minted on every claim, including redelivery, so token
identity alone encodes reservation validity: acknowledge() never needs a
...
class JobStore:
    ...
    async def take_fault(self) -> PendingFault | None:
        async with self._lock:
